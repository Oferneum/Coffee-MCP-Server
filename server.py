...
def ask(query: str) -> str:
    ...
    try:
        ...
        parts.append(
            "── RESPONSE FORMAT (MANDATORY — follow exactly, character for character) ──\n"
            "Audience: home barista, new to espresso. Write like a sharp, warm expert — not a textbook.\n\n"
            "Use EXACTLY this layout:\n\n"
            "## [PhysicsModel or root cause]\n"
            "[One sentence: scientific mechanism + what the barista tastes. "
            "E.g. 'When water races through the gaps between particles without slowing into their cores, "
            "it picks up colour and bitterness but leaves the sweetness behind.']\n\n"
            "---\n\n"
            "**[Specific setting name], [by how much / to what target]** — [what changes in the cup when this is right]. "
            "Write each adjustment as a single line in this exact format — no bullet dashes, no colons. "
            "Two or three adjustments, each on its own line with a blank line between.\n\n"
            "---\n\n"
            "*[One original metaphor. Make it tactile and unexpected — not fruit, not doors. "
            "Something from physics, texture, or light. Short.]*\n\n"
            "> **You'll know it's working when** [one sensory cue the barista can feel or see without equipment — "
            "crema behaviour, texture on the tongue, aftertaste length].\n\n"
            "If SOURCED_FROM edges appear in the retrieval, add:\n"
            "**Source:** [expert or paper name]\n"
            "Otherwise omit the Source line entirely — never invent a citation.\n\n"
            "Hard rules: ≤160 words. No filler openers. No prose paragraphs outside the opening sentence. "
            "No nested bullets. The `---` dividers and `>` blockquote are mandatory."
        )
